...
class ControlHandler:
    ...
    def handle_PORT(self, addr):
        ...
        if self.data_sock:
            # self.disconnect(self.data_sock)
            self.disconnect_data()
        server_addr = self.sock.getsockname()[0], 20
        client_addr = addr
        try:
            self.data_sock = self.server.connect(client_addr, server_addr)
    ...
    def handle_PASV(self, arg):
        ...
        if self.data_sock:
            self.disconnect_data()
        server_addr = self.server.create_data_listener(self)
        return '227 Entering Passive Mode. %s,%s' % server_addr

    def handle_MKD(self, dirname):
        path = '%s/%s' % (self.pwd, dirname)
        is_exist = os.path.isdir(path)
        if is_exist:
            raise HandleError('550 Requested action not taken.File unavailable')
        os.mkdir(path)
        return '257 "%s" created' % path

    def handle_CWD(self, dirname):
        is_exist = os.path.isdir(dirname)
        if not is_exist:
            raise HandleError('550 Requested action not taken.File unavailable')
        self.pwd = dirname
        return '250 Requested file action okay, completed.'
    ...
    def create_data_connect(self):
        self.initiative_data_connect()

    def initiative_data_connect(self):
        server_addr = self.sock.getsockname()[0], 20
    ...
    def login(self, user):
        ...
        self.server.login_users[user.name] = self
